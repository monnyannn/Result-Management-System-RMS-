...
form.addEventListener('submit', (e) => {
  ...
  inputEls.map((input) => {
    if (input.value && !editFlag) {
      ...
      const studentScores = Object.assign({}, ...arr)
      studentScores.id = 'qw' + new Date().getTime().toString()
      studentArr = JSON.parse(sessionStorage.getItem('studentScores')) || []
      studentArr.push(studentScores)
  ...
  sessionStorage.setItem('studentScores', JSON.stringify(studentArr))
  let data = JSON.parse(sessionStorage.getItem('studentScores'))
  ...
  for (let student of data) {
    studentBio.innerHTML += `
            <tr id='${student.id}' class="flex space-between wrap">
                  ...
                  <td>${student.test}</td>
                  <td class="d-none-mobile">${student.exam}</td>
                  <td class="d-none-mobile">${student.total}</td>
                  <td class="d-none-mobile">${student.grade}</td>
                  <td class="flex">
...
const displaystudentScores = () => {
  if (sessionStorage.getItem('studentScores')) {
    const studentArr = JSON.parse(sessionStorage.getItem('studentScores'))
    studentArr.forEach((student) => {
      ...
      studentBio.innerHTML += `
        <tr id='${id}' class="flex space-between wrap">
              ...
              <td>${course_code}</td>
              <td>${test}</td>
              <td class="d-none-mobile">${exam}</td>
              <td class="d-none-mobile">${total}</td>
              <td class="d-none-mobile">${grade}</td>
              <td class="flex">
              <button type="button" class="edit-btn"><img src="../images/editBtn.png" class="edit"></button>
              <button type="button" class="delete-btn"><img src="../images/trash.png" class="delete"></button>
              </td>
...
const removeFromsessionStorage = (id) => {
  let items = JSON.parse(sessionStorage.getItem('studentScores'))
  items = items.filter((item) => item.id !== id)
  sessionStorage.setItem('studentScores', JSON.stringify(items))
